...
public class TrainingScreen {
    ...
    private Label statsLabel, enemyStatsLabel;
    ...
    public Scene createScene() {
        ...
        trainStrBtn = new Button("Train STR");
        trainAgiBtn = new Button("Train AGI");
        trainIntBtn = new Button("Train INT");
    ...
    private void trainStat(String stat) {
        ...
        for(Enemy e : enemies) e.getStats().increaseRandomStat(5);
        updateStatsDisplay();
    ...
    private void updateStatsDisplay() {
        ...
        statsLabel.setText(String.format("YOUR STATS: STR:%d AGI:%d INT:%d HP:%d", s.getStrength(), s.getAgility(), s.getIntelligence(), s.getHp()));

        StringBuilder et = new StringBuilder("ENEMY STATS:\n");
        for(Enemy e : enemies) {
            Stat es = e.getStats();
            et.append(String.format("%s: STR:%d AGI:%d INT:%d HP:%d\n", e.getName(), es.getStrength(), es.getAgility(), es.getIntelligence(), es.getHp()));
        }
        enemyStatsLabel.setText(et.toString());
    }
}
